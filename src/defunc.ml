...
module Of_python = struct
  type 'a t = { type_name : string; conv : pyobject -> 'a }
...
module Opt_arg = struct
  type 'a t = { name : string; of_python : 'a Of_python.t; docstring : string }
end
...
let check_valid_arg_name name =
  if String.is_empty name then failwith "cannot use an empty name"
  else
    let first_char = name.[0] in
    if Char.(first_char < 'a' || first_char > 'z') then
      Printf.failwithf "arg name %s does not start with a lowercase letter" name
        ()
    else if String.exists name ~f:(fun c -> not (valid_char c)) then
      Printf.failwithf "arg name %s contains some invalid characters" name ()
    else ()

let apply (type a) (t : a t) args kwargs =
  let try_of_python v ~of_python ~name =
    try of_python.Of_python.conv v
    with e ->
      value_errorf "error processing arg %s (%s): %s" name of_python.type_name
        (Exn.to_string e)
  in
  ...
  let rec loop : type a. a t -> pos:int -> a * int =
   fun t ~pos ->
    ...
    | Arg { name; of_python; docstring = _; kind = `positional } ->
        if pos >= Array.length args then
          value_errorf "not enough arguments (got %d, expected %s)"
            (Array.length args)
            (positional_arguments () |> String.concat ~sep:", ");
        (try_of_python args.(pos) ~of_python ~name, pos + 1)
    | Opt_arg { name; of_python; docstring = _ } ->
        if Hash_set.mem kwnames name then
          value_errorf "multiple keyword arguments with name %s" name;
  ...
  Map.iter_keys kwargs ~f:(fun key ->
      if not (Hash_set.mem kwnames key) then
        value_errorf "unexpected keyword argument %s" key);
  if final_pos <> Array.length args then
    value_errorf "expected %d arguments (%s), got %d" final_pos
      (positional_arguments () |> String.concat ~sep:", ")
      (Array.length args);
  v
...
let params_docstring t =
  ...
  let rec loop : type a. a t -> pos:int -> string list * int =
   fun t ~pos ->
    ...
    | Both (t1, t2) ->
        ...
        (params1 @ params2, pos)
    | Arg ({ kind = `positional; _ } as arg) ->
        ([ arg_docstring arg ~pos ], pos + 1)
  ...
  if List.is_empty params then None
  else String.concat params ~sep:"\n\n" |> Option.some
...
module Param = struct
  ...
  let typerep tr =
    Of_python.create ~type_name:(Py_typerep.to_ocaml tr)
      ~conv:(Py_typerep.python_to_ocaml tr)

  let check_tuple_len pyobject ~expected_length =
    if not (Py.Tuple.check pyobject) then
      Printf.failwithf "expected a tuple got %s"
        (Py.Type.get pyobject |> Py.Type.name)
        ();
    let length = Py.Tuple.size pyobject in
    if expected_length <> length then
      Printf.failwithf "expected a tuple with %d elements, got %d"
  ...
  let triple (o1 : _ Of_python.t) (o2 : _ Of_python.t) (o3 : _ Of_python.t) =
    Of_python.create
      ~type_name:
        (Printf.sprintf "(%s, %s, %s)" o1.type_name o2.type_name o3.type_name)
      ~conv:(fun pyobject ->
  ...
  let list (o : _ Of_python.t) =
    Of_python.create ~type_name:(Printf.sprintf "[%s]" o.type_name)
      ~conv:(fun python_value ->
        ( match Py.Type.get python_value with
        | List | Tuple -> ()
        | otherwise ->
            Printf.failwithf "not a list or a tuple (%s)"
              (Py.Type.name otherwise) () );
  ...
  let list_or_iter (o : _ Of_python.t) =
    Of_python.create ~type_name:(Printf.sprintf "[%s]" o.type_name)
      ~conv:(fun p ->
        ...
        | None ->
            Printf.failwithf "not a list/tuple/iter (%s)"
  ...
  let one_or_tuple_or_list_relaxed (o : _ Of_python.t) =
    Of_python.create
      ...
      ~conv:
        (One_or_tuple_or_list_or_error.t_of_python o.conv
